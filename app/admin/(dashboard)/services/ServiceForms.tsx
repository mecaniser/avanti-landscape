...
/**
 * With every category always fully expanded, a busy service list means
 * scrolling past everything to find one thing — the exact complaint that
 * prompted this. A grid-template-rows 0fr/1fr transition animates open/close
 * against real content height with no JS measurement and no dependency on
 * <details>, which can't animate height on its own; the id-linked
 * aria-controls plus aria-expanded reproduce native disclosure semantics.
 */
export function CategorySection({
  ...
  const bodyId = `service-category-body-${id}`;

  return (
    <>
      <h3 className="service-admin-category__head">
        <button
          type="button"
          className="service-admin-category__head-btn"
          onClick={() => setOpen((value) => !value)}
          aria-expanded={open}
          aria-controls={bodyId}
        >
          <span className="service-admin-category__head-text">
            <span className="service-admin-category__head-title">{label}</span>
            <span className="service-admin-category__head-count">
              {count} {count === 1 ? "service" : "services"}
            </span>
          </span>
          <span className="service-admin-category__chevron" data-open={open}>
            <ChevronIcon />
          </span>
        </button>
      </h3>
      <div className="service-admin-category__collapse" data-open={open}>
        {/* grid-template-rows: 0fr + overflow: hidden is purely visual — it
            doesn't remove content from the tab order or accessibility tree
            on its own, so a keyboard user could tab through a collapsed
            category's hidden services. inert closes that, same fix as the
            mobile nav drawer's closed state. */}
        <div className="service-admin-category__collapse-inner" id={bodyId} inert={!open}>
          {children}
        </div>
      </div>
    </>
